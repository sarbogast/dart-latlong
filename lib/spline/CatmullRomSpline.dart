...
class CatmullRomSpline<T extends num> extends CatmullRom<double> {
  ...
  double position(final double distance) {
    assert(distance >= 0, "Distance must be beteen 0 and 1 but was $distance");
    assert(distance <= 1, "Distance must be beteen 0 and 1 but was $distance");
...
class CatmullRomSpline2D<T extends num> extends CatmullRom<Point2D<double>> {
  ...
  Point2D<double> position(final double distance) {
    assert(distance >= 0, "Distance must be between 0 and 1 but was $distance");
    assert(distance <= 1, "Distance must be between 0 and 1 but was $distance");
